...
# ===================================================================
# :: Stand Alone Report Base Class
# ===================================================================
class StandAloneReportBase(ABC):
    """
    Abstract Base Class for all Tower Stand Alone Reports.
    ...
    * ``_impl_generate_report``
    ...
    def FR_IDS(cls):
        """
        List of tuples identifying all of the Rules checked by this stand_alone_report.
    ...
    def INPUTS(cls):
        """
        List of tuples identifying all of the InputClients that this stand_alone_report accepts.
        ...
        * `True` means the stand_alone_report **requires** the input
        * `False` means the stand_alone_report will accept `None` if it's not available
    ...
    def generate_report(self, *args):
        ...
        :raises Exception: If the stand_alone_report has already completed its checks.
        """
        if self._check_complete:
            raise Exception(f'Stand alone report "{self.NAME}" has already completed checks!')
        self._impl_init(*args)
        self._impl_generate_report(*args)
    ...
    @abstractmethod
    def _impl_generate_report(self, *args):
        """
    ...
    def yield_rules(self):
        """
        Generator yielding all RuleResult objects managed by this stand_alone_report.
    ...
    def get_rule(self, rule_id):
        ...
        :rtype: RuleResults
        :raises ValueError: If the stand_alone_report does not manage the specified rule ID.
        """
        ...
        raise ValueError(f'Stand alone report "{self.NAME}" has no rule with ID {rule_id}')
